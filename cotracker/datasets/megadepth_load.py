...
from utils.image_process import read_megadepth_depth, read_rgb
import pickle
import torch
...
class MultiviewMatchingDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.image_tuples = image_tracks
    ...
    def __getitem__(self, index):
        ...
        for image_id in image_tuple:
            ...
            # Load depth
            # ts_depth = read_megadepth_depth(depth_path, original_hw, 
            #                                 (self.img_resize, self.img_resize), device=self.device)
            ts_depth = read_megadepth_depth(depth_path, pad_to=self.depth_max_size)
            
            # intrinsic_new = adjust_intrinsic(intrinsic, scale)
            # intrinsic_new = torch.from_numpy(intrinsic_new).to(torch.float32)
            # Append:
        ...
        depth_maps, intrinsics, poses, img_scales, img_original_hw = map(lambda x: torch.stack(x, dim=0).to(self.device), [depth_maps, intrinsics, poses, img_scales, img_original_hw])

        # Get relative poses:
        relative_poses = []
        for i in range(len(image_tuple)):
            if i == 0:
                relative_poses.append(torch.eye(4, device=self.device))
            else:
